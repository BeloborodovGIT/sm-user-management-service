...
from sqlalchemy import (
    BigInteger, Boolean, Date, ForeignKey, Index,
...
class User(Base):
    ...
    timezone_id: Mapped[int] = mapped_column(
        SmallInteger,
        ForeignKey("timezone_dict.id", ondelete="RESTRICT"),
        nullable=False,
    )
    username: Mapped[str] = mapped_column(
        String(60), nullable=False,
    )
    # оригинальная опечатка из схемы
    firtsname: Mapped[str] = mapped_column(
        String(60), nullable=False,
    )
    ...
    user_lock: Mapped[bool] = mapped_column(
...
class UserRole(Base):
    ...
    role_id: Mapped[int] = mapped_column(
        SmallInteger,
        ForeignKey("roles_dict.id", ondelete="RESTRICT"),
        nullable=False,
    )
...
class UserSending(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_user_sendings_status_id", "status_id",
        ),
        Index(
    ...
    status_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("status_dict.id", ondelete="RESTRICT"),
        nullable=False,
    )
    created_date: Mapped[date] = mapped_column(
        Date, nullable=False,
...
class UserReportLink(Base):
    ...
    __table_args__ = (
        Index(
            "idx_user_report_links_user_id", "user_id",
        ),
        Index(
    ...
    # оригинальная опечатка из схемы
    acive_from: Mapped[date] = mapped_column(
        Date, nullable=False,
    )
    active_to: Mapped[date | None] = mapped_column(Date)
