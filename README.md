...
Построен на FastAPI + SQLAlchemy (async) + PostgreSQL + Redis.
...
- **Alembic** — миграции
- **PostgreSQL 16**
- **Redis 7** — кеширование (redis-py + hiredis)
...
```
app/
├── main.py              # FastAPI-приложение, lifespan (Redis init), роутеры
├── config.py            # Настройки из .env / переменных окружения
├── database.py          # Async engine, Base, get_db()
├── cache.py             # Redis-кеш: CachedUser, user/superuser/dict кеши
├── auth/
...
Сервис использует Redis для кеширования горячих данных. Подключение инициализируется
при старте приложения и закрывается при остановке (lifespan).

| Ключ | TTL | Данные | Инвалидация |
|---|---|---|---|
| `user:{id}` | 5 мин | id, company_id, user_lock | update/delete user |
| `super:{id}` | 5 мин | bool (есть ли роль superuser) | delete user, assign role |
| `dict:all_roles` | 10 мин | JSON-список ролей | add/remove role function |
